...
class Rectangle():
    ...
    def contains(self,point : Point):
        return self.__top_left.x() <= point.x() <= self.__top_left.x()+self.__width and self.__top_left.y()<= point.y() <= self.__top_left.y()+self.__height
    ...
    def rebound(self, pose : Point, current_theta:float):
        ...
        if pose.y() <= self.__top_left.y() or pose.y() >= self.__top_left.y() + self.__height:
            new_theta = 2*math.pi - new_theta
        return new_theta

    def random_point(self, seed):
        x = random.uniform(self.__top_left.x(), self.__top_left.x() + self.__width)
        y = random.uniform(self.__top_left.y(), self.__top_left.y() + self.__height)
        return Point(x,y)
