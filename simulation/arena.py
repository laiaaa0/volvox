...
class Arena():
    def __init__(self, num_agents=5, pattern=Pattern.SQUARE):
        ...
        self.__pattern = np.zeros((self.__width, self.__height), dtype=np.uint8)
    ...
    def initialise_agents(self, num_agents : int, seed=42):
        ...
        for _ in range(num_agents):
            self.__agent_list.append(Agent(self.__rectangle, speed = 10*random.random(), theta=random.uniform(0,2*math.pi), pos = self.__rectangle.random_point(seed),
            frames_on=random.randint(20,50), frames_off=random.randint(3,5)))


    def image_from_pattern(self):
        coloured_pattern = np.ones((self.__width,self.__height,4), dtype=np.uint8)*255
    ...
    def is_position_illuminated(self,position:Point):
        ...
        transformed_position = position+Point(self.__width/2, self.__height/2)
